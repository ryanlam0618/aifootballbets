...
# -*- coding: utf-8 -*-
"""
数据转换脚本 - 将Football data crawling tool的比赛球员数据转换为big_five_data格式

功能:
1. 比赛技术统计 -> 转换为 matchlog.csv (按球队、按赛季)
2. 球员数据 -> 转换为 players.csv (按球队)
3. 保留中文队名
4. 自动翻译队名为英文 (在big_five_data目录结构中)
"""
...
def process_match_stats(source_file, output_dir):
    ...
    for idx, row in stats_df.iterrows():
        ...
        result_row = {
            ...
            'HalfCorner': row.get('HalfCorner', ''),
            'YellowCard': row.get('YellowCard', ''),
            'RedCard': row.get('RedCard', ''),
            'Shots': row.get('Shots', ''),
            'ShotsOnTarget': row.get('ShotsOnTarget', ''),
            'Attacks': row.get('Attacks', ''),
            'DangerousAttacks': row.get('DangerousAttacks', ''),
            'ShotsOffTarget': row.get('ShotsOffTarget', ''),
            'ShotsBlocked': row.get('ShotsBlocked', ''),
            'FreeKicks': row.get('FreeKicks', ''),
            'Possession': row.get('Possession', ''),
            'HalfPossession': row.get('HalfPossession', ''),
            'Passes': row.get('Passes', ''),
            'PassSuccessRate': row.get('PassSuccessRate', ''),
            'Fouls': row.get('Fouls', ''),
            'Offsides': row.get('Offsides', ''),
            'Headers': row.get('Headers', ''),
            'HeadersSuccessful': row.get('HeadersSuccessful', ''),
            'Saves': row.get('Saves', ''),
            'Tackles': row.get('Tackles', ''),
            'Dribbles': row.get('Dribbles', ''),
            'ThrowIns': row.get('ThrowIns', ''),
            'HitWoodwork': row.get('HitWoodwork', ''),
            'Interceptions': row.get('Interceptions', ''),
            'Blocks': row.get('Blocks', ''),
            'Assists': row.get('Assists', ''),
            'LongPasses': row.get('LongPasses', ''),
            'SuccessfulCrosses': row.get('SuccessfulCrosses', ''),
        }
...
def process_player_data(source_file, output_dir):
    ...
    for team_name_raw, group in player_df.groupby('Team Name'):
        ...
        # 选择球员特定的列，并翻译球队名
        available_cols = [c for c in player_columns if c in group.columns]
        new_df = group[available_cols].copy()
        new_df['MatchDate'] = date_str
        new_df['Opponent'] = opponent_en
        new_df['TeamName'] = team_name_en
        
        # 确保列顺序
        cols = ['MatchDate', 'Opponent', 'TeamName'] + available_cols
        new_df = new_df[cols]
