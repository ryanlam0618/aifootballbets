...
def parse_filename(filename):
    ...
    # 格式: 20250628_日职联_[3]广岛三箭_vs_名古屋鲸八[14]
    # 使用更灵活的解析
...
def process_player_data(source_file, output_dir):
    ...
    for team_name_raw, group in player_df.groupby('Team Name'):
        ...
        group = group.copy()
        group['MatchDate'] = date_str
        group['Opponent'] = opponent_en
        
        save_cols = ['MatchDate', 'Opponent', 'Team Name', 'Number', 'Player', 'Position', 
                     'Rating', 'Key Events', 'Shots', 'Shots on Target', 'Key Passes',
                     'Pass Success Rate', 'Aerial Duels Won', 'Physical Duels']
        save_cols = [c for c in save_cols if c in group.columns]
        
        if os.path.exists(players_file):
            existing_df = pd.read_csv(players_file)
            mask = ~((existing_df['MatchDate'] == date_str) & (existing_df['Opponent'] == opponent_en))
            existing_df = existing_df[mask]
            new_df = group[save_cols]
            combined_df = pd.concat([existing_df, new_df], ignore_index=True)
        else:
            combined_df = group[save_cols]
...
def convert_data():
    ...
    for file_path in files:
        ...
        try:
            ...
            if parsed:
                print(f"  日期: {parsed['date'].strftime('%Y-%m-%d')}, 联赛: {parsed['league']}")
                print(f"  主队: {parsed['home_team']} -> {translate_team_name(parsed['home_team'])}")
                print(f"  客队: {parsed['away_team']} -> {translate_team_name(parsed['away_team'])}")
